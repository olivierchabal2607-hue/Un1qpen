...
export function QuantitySelector({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  ...
  return <div>
    ...
    <input id="configurator-quantity" type="number" min={1000} step={1} value={value || ""} onChange={event => onChange(Number(event.target.value))} className="mt-3 min-h-12 w-full rounded-xl border border-[#c7c7cc] bg-white px-4 focus:border-[#17486a] focus:outline-none" aria-invalid={!valid} aria-describedby="quantity-error"/>
    {!valid && <p id="quantity-error" role="alert" className="mt-2 text-sm font-medium text-red-700">La quantité minimale est de 1 000 pièces.</p>}
  </div>;
}
