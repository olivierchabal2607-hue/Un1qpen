...
export function ContactForm() {
  ...
  if(status==="success") return <div className="rounded-3xl bg-[#e8eee9] p-10" role="status"><CheckCircle2 size={36}/><h2 className="mt-5 text-2xl font-semibold">Votre demande est bien arrivée.</h2><p className="mt-3 text-[#66645e]">Notre équipe reviendra vers vous prochainement.</p></div>;
  const inputClass="mt-2 w-full rounded-xl border border-[#c9c6be] bg-white px-4 py-3 focus:border-[#a46f52] focus:outline-none";
  return <form onSubmit={submit} noValidate className="grid gap-5 md:grid-cols-2">
    ...
    <button disabled={status==="loading"} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#151515] px-7 font-semibold text-white disabled:opacity-50 md:col-span-2 md:justify-self-start">{status==="loading"&&<Loader2 className="animate-spin" size={18}/>}Envoyer la demande</button>
  </form>;
}
