...
export function ContactForm() {
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setStatus("loading");setErrors({});const form=new FormData(e.currentTarget);const data:Record<string,FormDataEntryValue|boolean>=Object.fromEntries(form);data.consent=form.get("consent")==="on";
    const res=await fetch("/api/contact",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(data)});const json=await res.json();if(res.ok)setStatus("success");else{setStatus("error");setErrors(json.errors||{_form:json.error||"L’envoi a échoué. Réessayez dans quelques instants."});}
  }
  ...
  return <form onSubmit={submit} noValidate className="grid gap-5 md:grid-cols-2">
    ...
    {status==="error"&&<p role="alert" className="text-sm text-red-700 md:col-span-2">{errors._form||"Vérifiez les champs signalés."}</p>}
