import { isPricedQuantity, markingColorCounts } from "./pricing.mjs";
...
export function isValidQuantity(quantity) {
  return Number.isFinite(quantity) && isPricedQuantity(quantity);
}
...
export function restorePersistedState(value, initialState) {
  ...
  try {
    ...
    const restored = {
      ...initialState,
      ...parsed,
      quantity: isPricedQuantity(parsed.quantity) ? parsed.quantity : initialState.quantity,
      markingLocation: parsed.markingLocation === "body" ? "body" : "clip",
      markingColorCount: markingColorCounts.includes(parsed.markingColorCount) ? parsed.markingColorCount : initialState.markingColorCount,
      uploadedLogo: null,
      logoTransforms: {
...
export function validateQuoteDraft(state) {
  if (!isValidQuantity(state.quantity)) return "Choisissez une quantité proposée entre 1 000 et 50 000 pièces.";
